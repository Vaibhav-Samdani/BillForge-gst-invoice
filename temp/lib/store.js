...
const convertLineItemAmounts = async (items, fromCurrency, toCurrency, exchangeRates) => {
    ...
    return {
        ...
        updateItem: (id, item) => set((state) => {
            const newItems = state.items.map((i) => {
                if (i.id === id) {
                    const quantity = item.quantity !== undefined ? item.quantity : i.quantity;
        ...
        setCurrency: async (currency) => {
            ...
            try {
                ...
                const convertedItems = await convertLineItemAmounts(state.items, previousCurrency, currency, state.exchangeRates);
                const newState = {
                    ...state,
        ...
        setRecurringConfig: (config) => set((state) => ({
            ...state,
