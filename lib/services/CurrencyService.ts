...
import { Currency, ExchangeRate, CurrencyAmount, SUPPORTED_CURRENCIES } from '../types/invoice';
import { 
  convertCurrency, 
...
export class CurrencyService {
  ...
  async fetchExchangeRates(baseCurrency: string = 'USD'): Promise<ExchangeRate[]> {
    if (!this.isCurrencySupported(baseCurrency)) {
      throw new CurrencyError(
        `Base currency ${baseCurrency} is not supported`,
    ...
    try {
      ...
      throw new CurrencyError(
        `Network error while fetching exchange rates: ${error instanceof Error ? error.message : 'Unknown error'}`,
        'CURRENCY_API_UNAVAILABLE'
      );
    }
  }
   ...
   */
  async getExchangeRates(baseCurrency: string = 'USD', forceRefresh: boolean = false): Promise<ExchangeRate[]> {
  ...
  async convertAmount(
    ...
    if (!exchangeRate) {
      throw new CurrencyError(
        `Cannot find exchange rate from ${fromCurrency} to ${toCurrency}`,
   ...
   */
  getCachedRates(baseCurrency: string = 'USD'): ExchangeRate[] {
   ...
   */
  areCachedRatesFresh(baseCurrency: string = 'USD'): boolean {
   ...
   */
  getCacheStatus(): { [baseCurrency: string]: { lastUpdate: Date; rateCount: number; isFresh: boolean } } {
    const status: { [key: string]: { lastUpdate: Date; rateCount: number; isFresh: boolean } } = {};
   ...
   */
  validateExchangeRate(rate: ExchangeRate): { isValid: boolean; errors: string[] } {
    const errors: string[] = [];
